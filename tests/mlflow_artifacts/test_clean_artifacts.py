...
def test_cleaning_metrics_logged(valid_hotel_df, mlflow_tracking_uri):
...
def test_rows_before_gte_rows_after(dirty_hotel_df, mlflow_tracking_uri):
    """Logged rows_before >= rows_after."""
    mlflow.set_experiment("test_artifacts")
    with mlflow.start_run() as run:
        clean(dirty_hotel_df)

    metrics = mlflow.get_run(run.info.run_id).data.metrics
    assert metrics["rows_before"] >= metrics["rows_after"]


def test_rows_dropped_equals_difference(dirty_hotel_df, mlflow_tracking_uri):
...
def test_cleaned_csv_artifact_logged(valid_hotel_df, mlflow_tracking_uri):
    """Cleaned CSV artifact is logged under cleaned_data/."""
    mlflow.set_experiment("test_artifacts")
    with mlflow.start_run() as run:
        clean(valid_hotel_df, cleaned_filename="cleaned_hotel_bookings.csv")

    client = mlflow.tracking.MlflowClient()
    artifacts = [a.path for a in client.list_artifacts(run.info.run_id, "cleaned_data")]
    assert any("cleaned_hotel_bookings.csv" in a for a in artifacts)


def test_raw_csv_artifact_logged(raw_csv_with_leaky, mlflow_tracking_uri):
